...
//        print_r($this->poitns);
     ...
     * @return Spot
     */
    private function makeChanceforNextMove(): void
    {
        $chanceSeries = $this->board->getSeriesByStatsIndex(201);

        foreach ($chanceSeries as $serie) {
            $this->addPoint($serie->getSpots(), 10);
        }
    }
    ...
    private function makeChanceAndDestroyOpponentChance()
    {
        ...
        if (count($series) > 1) {
            $this->addPoint($this->checkJunktions($series), 3);
        }
    }
